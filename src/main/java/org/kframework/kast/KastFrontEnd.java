...
public class KastFrontEnd extends FrontEnd {
    ...
    public static Module[] getModules(String[] args) {
        try {
            ...
            return new Module[] { new KastModule(options), new JCommanderModule(args),
                    new CommonModule() };
    ...
    @Inject
    KastFrontEnd(KastOptions options, @Main Provider<Context> contextProvider,
            @Usage String usage, @ExperimentalUsage String experimentalUsage, Stopwatch sw,
    ...
    public boolean run() {
        ...
        try {
            ASTNode out = ProgramLoader.processPgm(stringToParse.getBytes(), source, sort,
                    context, options.parser);
            StringBuilder kast;
            if (options.experimental.pretty) {
                IndentationOptions indentationOptions = new IndentationOptions(
                        options.experimental.maxWidth(), options.experimental.auxTabSize,
                        options.experimental.tabSize);
                KastFilter kastFilter = new KastFilter(indentationOptions,
                        options.experimental.nextLine, context);
